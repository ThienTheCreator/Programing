...
public:
    Node* cloneGraph(Node* node) {
        ...
        for(int i = 0; i < node->neighbors.size(); i++){
            Node* clone = cloneGraph(node->neighbors[i]);
...
m is the size of the hashmap and n is number of nodes
Time: O(m + n)
Space: O(n)

Wasn't able to solve this question in 20 minutes because I did not realize that I need a hashmap
for the neighbors. The hashmap stores the node reference if he have created it already instead of
creating a new one. The connection goes to each neighbor and check if we have visited it already.
