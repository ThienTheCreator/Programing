...
It is guaranteed that each 'Sell' operation for a stock has a corresponding 'Buy' operation in a
previous day. It is also guaranteed that each 'Buy' operation for a stock has a corresponding
'Sell' operation in an upcoming day.
...
FROM
    (SELECT 
    stock_name,
    IF(operation = "Buy", -1 * price, price) AS capital_gain_loss
    FROM Stocks) t1 
GROUP BY 
stock_name;
...
I selected from the table filter out the result and selected from the table again.
Another solution is 
