...
public:
    ...
    string get(string key, int timestamp) {
        ...
        int low = 0;
        int high = um[key].size() - 1;

        while(low <= high){
            int mid = low + (high - low) / 2;
            
            if(um[key][mid].first < timestamp){
                low = mid + 1;
            }else if(um[key][mid].first > timestamp){
                high = mid - 1;
            }else{
...
Couldn't solve the problem. I was unaware that timestamps are strictly increasing. Also had error
in logic.

Solution is from neet code.
Time: O(log n)
Space: O(n)

Solution uses unordered map of string and vector. The vector stores an int and a string.
Map searches if key is it then does a binary search on the vector stored as the value.
