/*

Given the root of an n-ary tree, return the postorder traversal of its nodes' values.

Nary-Tree input serialization is represented in their level order traversal. Each group of children
is separated by the null value (See examples)

Example 1:

Input: root = [1,null,3,2,4,null,5,6]
Output: [5,6,3,2,4,1]

Example 2:

Input: root = 
[1,null,2,3,4,5,null,null,6,7,null,8,null,9,10,null,null,11,null,12,null,13,null,null,14]
Output: [2,6,14,11,7,3,12,8,4,13,9,10,5,1]
 
Constraints:

The number of nodes in the tree is in the range [0, 104].
0 <= Node.val <= 104
The height of the n-ary tree is less than or equal to 1000.

*/

/*
// Definition for a Node.
class Node {
public:
    int val;
    vector<Node*> children;

    Node() {}

    Node(int _val) {
        val = _val;
    }

    Node(int _val, vector<Node*> _children) {
        val = _val;
        children = _children;
    }
};
*/

class Solution {
public:
    vector<int> postorder(Node* root) {
        vector<int> res;
        vector<int> temp;
        
        if(root != nullptr){
            for(auto r : root->children){
                temp = postorder(r);
                res.insert(res.end(), temp.begin(), temp.end());
            }
            
            res.push_back(root->val);
        }
        
        return res;
    }
};

/* Note

I used a recursive solution which was much faster to implement. I saw a iterative solution which is
interesting. 

The solution is from MichealZ:
https://leetcode.com/problems/n-ary-tree-postorder-traversal/discuss/150339/C%2B%2B-super-easy-11-line-solution-beats-100!

vector<int> postorder(Node* root) {
    if(root==NULL) return {};
    vector<int> res;
    stack<Node*> stk;
    stk.push(root);
    while(!stk.empty())
    {
        Node* temp=stk.top();
        stk.pop();
        for(int i=0;i<temp->children.size();i++) stk.push(temp->children[i]);
        res.push_back(temp->val);
    }
    reverse(res.begin(), res.end());
    return res;
}

What is interesting is that he uses a stack which confuse me at first on how the children are
traverse. The stack will take the right children first and for the final solution, it reverses
the vector.

*/