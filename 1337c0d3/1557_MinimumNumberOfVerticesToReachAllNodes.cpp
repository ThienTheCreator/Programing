...
I thought that this problem would require dfs, but this problem was easier to solve then I thought.
I looked at the discussion to see if it used dfs, but it just counted visited node and return all
the nodes not visited which is the nodes that cannot be visited. Since the graph is acyclical it
can be simplified based on the conditions given in the questions.

The solution by lee215 is from 
