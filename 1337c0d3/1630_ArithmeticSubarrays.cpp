...
representing the m range queries, where the ith query is the range [l[i], r[i]]. All the arrays are
0-indexed.
...
I was looking at the most voted solution and votrubac suggested two solution one was by sorting and
the other was by figuring out the pattern. First we find the min and the max element in the array.
Then we generate another array and use the pattern and loop through the subarray to see if it fits
the pattern. If it doesn't fit the pattern then break from the loop. The pattern was (num - min)/d. 
